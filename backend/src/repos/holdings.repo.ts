...
export interface Lot {
  ...
  plan_id: number | null; // dca_plans.id if auto-generated by a 定期定額 plan
  created_at: string;
}
...
export function addLot(lot: NewLot): Lot {
  const info = run(
    `INSERT INTO holding_lots
       (ticker, shares, cost_basis, currency, purchase_date, notes, target_price, stop_loss, plan_id, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    lot.ticker,
    lot.shares,
    lot.cost_basis,
  ...
  run(
    `UPDATE holding_lots
     SET ticker = ?, shares = ?, cost_basis = ?, currency = ?, purchase_date = ?,
         notes = ?, target_price = ?, stop_loss = ?, plan_id = ?
     WHERE id = ?`,
    next.ticker,
    next.shares,
    next.cost_basis,
...
export function lotsByPlan(planId: number): Lot[] {
  return all<Lot>('SELECT * FROM holding_lots WHERE plan_id = ? ORDER BY purchase_date, id', planId);
}

export function deleteLotsByPlan(planId: number): number {
  return Number(run('DELETE FROM holding_lots WHERE plan_id = ?', planId).changes);
}
