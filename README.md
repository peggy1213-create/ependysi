...
```
Investment/
├── package.json            # workspaces + dev/build/app/dist scripts + electron-builder config
├── electron/
│   ├── main.cjs            # boots the Express backend in-process, opens the window
│   └── resources/icon.ico  # generated by make-icon.mjs
├── config/
...
The dashboard also runs as a native Windows app — no terminal, backend bundled
in-process, data stored in `%APPDATA%\Investment Dashboard\data\`.

```bash
npm run app     # build + launch the Electron app (dev)
npm run dist    # build the installer → release\Investment Dashboard Setup 0.1.0.exe
```

Double-click the installer to install; it adds a desktop + Start-menu shortcut.
The app finds a free port and opens the dashboard in its own window; news links
open in your default browser. **Data menu → Open data folder** shows where the
SQLite file lives.
...
- **`npm run dist` on Windows** needs symlink permission for one build dependency
  ...
  neither. Builds are unsigned (Windows SmartScreen will warn on first run —
  "More info → Run anyway").
- No custom `productName`/publisher signing → the installer is not code-signed.
...
| `npm run start`                      | Run the compiled backend (`backend/dist`, serves the API only) |
| `npm run app` / `npm run dist`       | Electron app (dev) / build the Windows installer   |
